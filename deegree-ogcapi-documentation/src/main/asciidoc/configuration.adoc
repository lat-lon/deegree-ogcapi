...
The following chapters describe how to set up a deegree ogcapi workspace by the given example.
...
<Datasets xmlns="http://www.deegree.org/ogcapi/datasets"
          ...
          xsi:schemaLocation="http://www.deegree.org/ogcapi/datasets  https://schemas.deegree.org/ogcapi/2.0/datasets.xsd">
...
<deegreeOAF xmlns="http://www.deegree.org/ogcapi/features"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            xsi:schemaLocation="http://www.deegree.org/ogcapi/features https://schemas.deegree.org/ogcapi/2.0/features.xsd">
  ...
  <QueryMaxItems>2000</QueryMaxItems>  <!--4-->

  <HtmlViewId>streetview</HtmlViewId>  <!--5-->
...
<4> configuration to set the value of the `limit` query parameter (optional)
<5> identifier of the HTML encoding configuration, links to file _html/streetsview.xml_
...
<deegreeOAF xmlns="http://www.deegree.org/ogcapi/features"
  ...
  <QueryMaxItems>2000</QueryMaxItems>  <!--5-->
  ...
  <HtmlViewId>treesview</HtmlViewId>  <!--8-->
  ...
  <Metadata>
    ...
    <MetadataURL format="application/xml">http://example.metadata.org?service=CSW&amp;request=GetRecordById&amp;version=2.0.2&amp;id=1234</MetadataURL> <!--12-->
    <MetadataURL format="text/html">http://example.metadata.org/path_to_html/1234</MetadataURL> <!--13-->
  </Metadata>

  <ConfigureCollection id="TreeFeature"> <!--14-->
    <AddLink href="https://inspire.ec.europa.eu/featureconcept/" rel="tag" type="text/html" title="Feature concept for trees"/>
  </ConfigureCollection>

  <ConfigureCollections>
    <AddLink href="https://github.com/INSPIRE-MIF/" rel="describedby" type="text/html" title="Encoding example"/> <!--15-->
    <AddLink href="https://schemas.deegree.org/trees.xsd" rel="describedby" type="application/xml" title="GML application schema for trees"/> <!--16-->
  </ConfigureCollections>
...
<5> configuration to set the value of the `limit` query parameter (optional)
<6> DateTime property defines a property _app:seedyear_ of the feature type _app:trees_ as a datetime property
<7> Geometry property defines a geometry property _app:geom1_ of the feature type _app:trees_ as the property to export as geometry in GeoJSON. Only required if the featureType contains multiple geometries. If the attribute _skipExportAsWkt_ is set to false, other geometries are exported as WKT in GeoJSON.
<8> identifier of the HTML encoding configuration, links to file _html/treesview.xml_
<9> provider license applicable to the service provider with description element
<10> dataset license applicable to the dataset using link to license element
<11> dataset provider contact details
<12> metadata link in format `application/xml` for the dataset (optional)
<13> metadata link in format `text/html` for the dataset (optional)
<14> configure additional links for an individual collection. In the example, an additional link to the INSPIRE feature concept for the collection is provided (optional) (required by INSPIRE)
<15> configure additional links for all collections. In the example, an additional link to the alternative encoding description for collections is provided (optional) (recommended by INSPIRE)
<16> configure additional links for all collections. In the example, an additional link to the GML application schema is provided (optional) (used by tools such as QGIS and GDAL)
...
|QueryMaxItems |0..1 |Integer |Configuration of the maximum allowed value for the query parameter `limit`, default is `1000` if not set
|DateTimeProperties |0..1 |Complex |Configuration of date and time properties, see https://docs.ogc.org/is/17-069r3/17-069r3.html#_parameter_datetime[parameter datetime in the OGC API specification] for more information
...
<deegreeServicesMetadata xmlns="http://www.deegree.org/services/metadata"
                         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                         xsi:schemaLocation="http://www.deegree.org/services/metadata https://schemas.deegree.org/core/3.6/services/metadata/metadata.xsd">
...
<HtmlView xmlns="http://www.deegree.org/ogcapi/htmlview"
          xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
          xsi:schemaLocation="http://www.deegree.org/ogcapi/htmlview https://schemas.deegree.org/ogcapi/2.0/htmlview.xsd">
...
|===
|Option |Cardinality |Value |Description
|WMSUrl |1 |URL |WMS service endpoint URL, default: https://sg.geodatenzentrum.de/wms_dtk250. Use the attribute `version` to specify the WMS version
|WMSLayers |1 |String |Name of the layer, default: dtk250
|CrsProj4Definition |1 |String |Use the attribute `code` to set the EPSG code, and the value element for the https://proj.org[PROJ] definition as provided by https://epsg.io.
|===
...
=== Disable links to download all features

By default, collections include the links "Download all features as GeoJSON" and "Download all features as GML".
To hide these links set the system property `deegree.oaf.links.enclosure.enabled` to _false_. The default is _true_.
This setting only controls link visibility. Direct requests with `bulk=true` remain available.

[[config_logging]]
=== Logging configuration

The deegree ogcapi is using the https://logback.qos.ch/[Logback Project] for logging. By default, deegree ogcapi uses
the deegree autoconfiguration submodule. This module can log to the console and to files, adjust log patterns, and define
log levels for various components of deegree ogcapi and its dependencies. If more advanced or customized logging is required,
you can supply a Logback XML configuration file, which disables autoconfiguration and enables even complex logging setups.
...
Further details on configuring both the autoconfiguration module and a file-based Logback setup can be found in the
https://download.deegree.org/documentation/current/html/#anchor-logging-configuration[documentation for deegree webservices].
