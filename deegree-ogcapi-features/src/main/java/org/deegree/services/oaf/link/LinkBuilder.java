...
public class LinkBuilder {
	...
	public List<Link> createDatasetLinks(String datasetId) {
		List<Link> links = new ArrayList<>();
		String datasetHref = createBaseUriBuilder(datasetId).toString();
	...
	/**
	 * @param datasetId id of the dataset, never <code>null</code>
	 * @param metadata describing this service, never <code>null</code>
	 * @return list of links, never <code>null</code>
	 */
	public List<Link> createLandingPageLinks(String datasetId, DatasetMetadata metadata) {
		...
		String conformanceHref = createBaseUriBuilder(datasetId).path("conformance").toString();
		addConformance(links, conformanceHref);

		String collectionsHref = createBaseUriBuilder(datasetId).path("collections").toString();
	...
	public List<Link> createCollectionLinks(String datasetId, String collectionId, List<MetadataUrl> metadataUrls) {
		...
		String itemsHref = createBaseUriBuilder(datasetId).path("collections")
			.path(collectionId)
			.path("items")
			.toString();
		addItems(links, itemsHref);
		String enclosureHref = createBaseUriBuilder(datasetId).path("collections")
			.path(collectionId)
			.path("items")
	...
	public String createSchemaLink(String datasetId, String collectionId) {
		return createBaseUriBuilder(datasetId).path("collections").path(collectionId).path("appschema").toString();
	}
	...
	private void addConformance(List<Link> links, String conformanceHref) {
		...
		links.add(new Link(conformanceHref, CONFORMANCE.getRel(), TEXT_HTML, "OGC API conformance classes as HTML"));
		links.add(
				new Link(conformanceHref, CONFORMANCE.getRel(), APPLICATION_XML, "OGC API conformance classes as XML"));
	}
	...
	private void addData(List<Link> links, String collectionsHref) {
		links.add(new Link(collectionsHref, DATA.getRel(), APPLICATION_JSON, "Supported Feature Collections as JSON"));
		links.add(new Link(collectionsHref, DATA.getRel(), TEXT_HTML, "Supported Feature Collections as HTML"));
		links.add(new Link(collectionsHref, DATA.getRel(), APPLICATION_XML, "Supported Feature Collections as XML"));
	}
	...
	private void addLicenseLink(String datasetId, DatasetMetadata metadata, List<Link> links) {
		if (metadata != null && metadata.getDatasetLicense() != null) {
			...
			if (datasetLicense.getUrl() != null) {
				...
				String licenseHref = createBaseUriBuilder(datasetId).path("license").path("dataset").toString();
