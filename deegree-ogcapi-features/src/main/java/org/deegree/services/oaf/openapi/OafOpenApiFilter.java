...
public class OafOpenApiFilter extends AbstractSpecFilter {
	...
	public static final List<String> SUPPPORTED_GEOM_TYPES = Arrays.asList("Point", "LineString", "Polygon",
	...
	private void filterPaths(OpenAPI openAPI) {
		...
		addMediaTypes(features, APPLICATION_GML, APPLICATION_GML_32, APPLICATION_GML_SF0, APPLICATION_GML_SF2,
				TEXT_HTML);
	...
	private Schema createFeatureResponseSchema(FeatureType featureType) {
		...
		Schema type = new Schema().name("type").type("string");
		type.addEnumItemObject("FeatureCollection");
		Schema numberMatched = new Schema().name("numberMatched").type("integer").example(178);
		Schema numberReturned = new Schema().name("numberReturned").type("integer").example(10);
		Schema timeStamp = new Schema().name("timeStamp").type("string").example("2020-12-14T09:32:42.669Z");
		Schema links = new ArraySchema().items(new Schema().$ref("#/components/schemas/Link")).name("links");
	...
	private void addPropertiesSchema(FeatureType featureType, Schema<Object> schema) {
		Schema properties = new Schema().name("properties").type("object");
	...
	private Schema createSchemaForProperty(PropertyType propertyType) {
		...
		if (propertyType instanceof SimplePropertyType && isMaxOccursGreaterThanOne(propertyType)) {
			return new ArraySchema().items(new Schema().type(mapToPropertyType(propertyType)))
		...
		if (propertyType instanceof CustomPropertyType customPropertyType && !isGmlProperty(propertyType)) {
			...
			if (isMaxOccursGreaterThanOne(propertyType)) {
				Schema arraySchema = new ArraySchema().items(new Schema().type(mapToPropertyType(propertyType)))
					.name(propertyType.getName().getLocalPart())
					.type("array");
	...
	private Schema createGeometryPropertySchema() {
		...
		typeEnum.setDescription(
				"Type of the geometry, one of " + SUPPPORTED_GEOM_TYPES.stream().collect(Collectors.joining(", ")));
		typeEnum.setEnum(SUPPPORTED_GEOM_TYPES);
		properties.put("type", typeEnum);
		Schema coordinatesSchema = new ArraySchema().items(new Schema().type("number"))
		...
		return new Schema().name(GEOMETRY_PROPERTY_NAME)
			.description("Geometry (" + SUPPPORTED_GEOM_TYPES.stream().collect(Collectors.joining(", "))
					+ ") as specified in RFC 7946.")
			.type("object")
